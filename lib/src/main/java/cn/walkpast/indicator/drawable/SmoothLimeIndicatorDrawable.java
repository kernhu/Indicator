...
public class SmoothLimeIndicatorDrawable extends Drawable implements Animatable {
  ...
  private SmoothLimeIndicatorDrawable(Interpolator interpolator,
                                      int sectionsCount,
                                      int separatorLength,
                                      int[] colors,
                                      float strokeWidth,
  ...
  public static class Builder {
    ...
    public SmoothLimeIndicatorDrawable build() {
      if (mGenerateBackgroundUsingColors) {
        mBackgroundDrawableWhenHidden = SmoothLimeIndicatorUtils.generateDrawableWithColors(mColors, mStrokeWidth);
      }
      SmoothLimeIndicatorDrawable ret = new SmoothLimeIndicatorDrawable(
          mInterpolator,
          mSectionsCount,
    ...
    private void initValues(Context context, boolean editMode) {
      ...
      if (!editMode) {
        mSectionsCount = res.getInteger(R.integer.sli_default_sections_count);
        mSpeed = Float.parseFloat(res.getString(R.string.sli_default_speed));
        mReversed = res.getBoolean(R.bool.sli_default_reversed);
        mProgressiveStartActivated = res.getBoolean(R.bool.sli_default_progressiveStart_activated);
        mColors = new int[]{res.getColor(R.color.sli_default_color)};
        mStrokeSeparatorLength = res.getDimensionPixelSize(R.dimen.sli_default_stroke_separator_length);
        mStrokeWidth = res.getDimensionPixelOffset(R.dimen.sli_default_stroke_width);
      } else {
        mSectionsCount = 4;
        mSpeed = 1f;
        mReversed = false;
    ...
    public Builder interpolator(Interpolator interpolator) {
      SmoothLimeIndicatorUtils.checkNotNull(interpolator, "Interpolator");
      mInterpolator = interpolator;
      return this;
    }

    public Builder sectionsCount(int sectionsCount) {
      SmoothLimeIndicatorUtils.checkPositive(sectionsCount, "Sections count");
      mSectionsCount = sectionsCount;
      return this;
    }

    public Builder separatorLength(int separatorLength) {
      SmoothLimeIndicatorUtils.checkPositiveOrZero(separatorLength, "Separator length");
    ...
    public Builder colors(int[] colors) {
      SmoothLimeIndicatorUtils.checkColors(colors);
      mColors = colors;
      return this;
    }

    public Builder strokeWidth(float width) {
      SmoothLimeIndicatorUtils.checkPositiveOrZero(width, "Width");
      mStrokeWidth = width;
      return this;
    }

    public Builder speed(float speed) {
      SmoothLimeIndicatorUtils.checkSpeed(speed);
      mSpeed = speed;
      return this;
    }

    public Builder progressiveStartSpeed(float progressiveStartSpeed) {
      SmoothLimeIndicatorUtils.checkSpeed(progressiveStartSpeed);
      mProgressiveStartSpeed = progressiveStartSpeed;
      return this;
    }

    public Builder progressiveStopSpeed(float progressiveStopSpeed) {
      SmoothLimeIndicatorUtils.checkSpeed(progressiveStopSpeed);
      mProgressiveStopSpeed = progressiveStopSpeed;
      return this;
    }
